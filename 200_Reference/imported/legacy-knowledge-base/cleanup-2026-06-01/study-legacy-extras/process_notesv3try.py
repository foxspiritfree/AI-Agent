# -*- coding: utf-8 -*-
"""
DEPRECATED: This legacy profile-specific entrypoint is kept as a backup.
Use ../_pipeline/zettelkasten_pipeline.py --profile study for new runs.
...
API_RATE_LIMIT_DELAY = 3 # Increased delay for larger batch requests
BATCH_SIZE = 10 # 每次 API 請求處理的筆記數量
...
def process_batch(batch_files: List[str], tree: Dict[str, Any], model: genai.GenerativeModel, existing_ids: Set[str], id_to_path_map: Dict[str, str]):
    ...
    try:
        ...
        for item in first_pass_items:
            new_id = finalize_note_processing(item['result'], item['note'], tree, existing_ids, id_to_path_map)
            if new_id:
        ...
        for item in second_pass_items:
            ...
            try:
                ...
                if real_parent_id:
                    ...
                    finalize_note_processing(item['result'], item['note'], tree, existing_ids, id_to_path_map)
                else:
...
def finalize_note_processing(result_data: Dict[str, Any], note: Dict[str, str], tree: Dict[str, Any], existing_ids: Set[str], id_to_path_map: Dict[str, str]) -> Optional[str]:
...
def main():
    """程式主進入點，協調整個流程。"""
    print("--- Zettelkasten Auto-Classifier v5.0 (Batch Mode) ---")
